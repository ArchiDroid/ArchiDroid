...
# shellcheck disable=2009,2039
if [[ "$(ps w | grep -qi "[d]ata/media/"; echo $?)" -eq 0 || "$(ps w | grep -qi "[s]dcard/"; echo $?)" -eq 0 ]]; then
	# Definitely intsd
	exit 0
elif [[ "$(ps w | grep -qi "[e]xt"; echo $?)" -eq 0 || "$(ps w | grep -qi "[s]dcard1/"; echo $?)" -eq 0 ]]; then
	# Definitely extsd
	exit 1
fi
